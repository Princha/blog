...
$(document).ready(function () {
    ...
    function get_stream () {
        if(auto_refresh){
            $.get('./api/data_stream?count=1000', function (result, status) {
                if(result.result == 0){
    ...
    function post_stream (){
        ...
        $.post('./api/data_stream?count=1000', {id:id, name:name, value:stream}, function (result, status) {
            if(result.result == 0){
                // console.log('0',result.data);
                $('#dialog').modal('hide');
            }else if(result.result == -1){
        ...
        $('#Table').bootstrapTable({
            ...
            queryParams: function queryParams(params) {   //设置查询参数  
                var param = {    
                    ...
                    search_sname: $('#sname-search-input').val(),
                    search_mode: $('#search-time-select').val()
                };
            ...
            columns:[{
                ...
                field:'name',
                title:language.table_column_sname,
                align: 'left',
                valign: 'middle',
                sortable: false
            },{
                field:'value',
                title:language.table_column_stream,
                align: 'left',
                valign: 'middle',
                sortable: false,
    ...
    // $(document).ready(function () {
        ...
        $('#sname-search-input').bind('keypress',function(event){   
            if(event.keyCode == 13){  
                $('#search-button').click(); 
            }  
        });
